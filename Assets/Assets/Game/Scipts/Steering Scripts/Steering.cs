...
public class Steering : MonoBehaviour
{
	...
	public Vector3 AvoidObstacle (GameObject obst, float safeDistance)
	{
		...
		//return zero vector if we can pass safely to top
		float topDotVTC = Vector3.Dot (vecToCenter, transform.up);
		if(Mathf.Abs (topDotVTC) > 20)
			return dv;
		
		if(topDotVTC > 0)
			dv = transform.up * -maxSpeed * safeDistance / dist;
		else
			dv = transform.up * maxSpeed * safeDistance / dist;
		
		//return zero vector if we can pass safely to right
		float rightDotVTC = Vector3.Dot (vecToCenter, transform.right);
		if (Mathf.Abs (rightDotVTC) > 20)
			return dv;
	...
	public Vector3 AvoidEnemyShip (GameObject obst, float safeDistance)
	{
		Vector3 dv = Vector3.zero;
		//compute a vector from charactor to center of obstacle
		Vector3 vecToCenter = obst.transform.position - transform.position;
		...
		//return zero vector if we can pass safely to top
		float topDotVTC = Vector3.Dot (vecToCenter, transform.up);
		if(Mathf.Abs (topDotVTC) > 10)
			return dv;
		
		//obstacle is up or down
		if(topDotVTC > 0)
			dv = transform.up * -maxSpeed * safeDistance / dist;
		else
			dv = transform.up * maxSpeed * safeDistance / dist;
